git config --global user.email "nfcutler@umich.edu"
git config --global user.name "Naomi Cutler"
**EAS648 Lab01**
**By Naomi Cutler**
...
```{r, fig.cap='This plot shows the distance to lakes, rivers, and creeks (hydro) for the Flickr and Panramio photo points. Panramio has a slighly larger spread and a higher mean (1947.38 m) than the Flickr distances (1734.822 m)'}
boulder %>%
...
```{r, Fig.cap='This map shows the Boulder data elevation using the Magma color scale which is more suitable for folks with visual impairments which may not allow them to distinguish between colors.'}
ggplot() +
    geom_sf(data = boulder, aes(color=PT_Elev),
    fill = NA, alpha = .2) + 
...
```{r, fig.cap='This map uses the tmap package to map the variable Climb_dist which is the distance to the nearest climbing wall from each photo point. The map splits the column values into 5 equal parts using "quantile" to display the data in different classes.'}
